...
const selectedBeerSimilarBeersIdsLens = lensPath([
  selectedBeerPath,
  "similarBeersIds",
]);
const selectedBeerSimilarErrorLens = lensPath([
  selectedBeerPath,
  "similarBeersError",
]);
...
const beersReducer = handleActions(
  {
    [combineActions(getBeersSuccess, getSimilarBeersSuccess)]: (
      state,
      { payload },
    ) => {
    ...
    [getSimilarBeersSuccess]: (state, { payload }) => {
      ...
      const beersWithoutSelected = filter(
      ...
      return over(
        selectedBeerSimilarBeersIdsLens,
        compose(
          concat(__, indexedBeersKeys),
          defaultTo([]),
        ),
...
const beersSelector = view(
  compose(
    beersReducerLens,
    beersLens,
  ),
);
const beersOrderSelector = view(
  compose(
    beersReducerLens,
    beersOrderLens,
  ),
);
const selectedBeerIdSelector = view(
  compose(
    beersReducerLens,
    selectedBeerIdLens,
  ),
);
const similarBeersIdsSelector = view(
  compose(
    beersReducerLens,
    selectedBeerSimilarBeersIdsLens,
  ),
);
const similarBeersErrorSelector = view(
  compose(
    beersReducerLens,
    selectedBeerSimilarErrorLens,
  ),
);
const hasAllBeersSelector = view(
  compose(
    beersReducerLens,
    hasAllBeersLens,
  ),
